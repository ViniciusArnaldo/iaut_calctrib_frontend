...
export const NBSTab: React.FC = () => {
  ...
  const { data, isLoading, refetch } = useNBSList(filters);
  const importMutation = useImportNBSFromGoverno();
  ...
  return (
    <>
      <ConfirmDialog
        ...
        message="Deseja sincronizar os dados de NBS com o governo? Isso pode levar alguns minutos e irá atualizar todos os registros."
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {isLoading ? (
                ...
                data.itens.map((nbs) => (
                  <tr key={nbs.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                      {nbs.codigoFormatado || nbs.codigo}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">
                      <div className="max-w-md truncate">{nbs.descricao}</div>
                    </td>
